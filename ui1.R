...
sd_question <- function(
  ...
  # Added this line -Zain
  output_div <- add_auto_scroll(output_div)
...
#'
#' This function adds an event listener to scroll to the next question when an answer is selected.
#'
#' @param output The UI output element to which we're adding the auto-scroll functionality.
#' @return The modified UI output element with auto-scroll functionality.
#'
#' @keywords internal
add_auto_scroll <- function(output) {
  script <- shiny::tags$script(HTML(
    "
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function handleAnswerSelection(event) {
        setTimeout(scrollToNextQuestion, 300);
      }
  ...
  return(shiny::tagList(output, script))
}